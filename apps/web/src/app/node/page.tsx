...
export default function WebNodePage() {
  ...
  const [logs, setLogs] = useState<
    { time: string; msg: string; type: LogType }[]
  >([]);
  ...
  const activateNode = () => {
    try {
      ...
      addLog(
        "Keep this tab open. Android will ask you to confirm each SMS before sending.",
        "info",
      );
  ...
  const fetchPendingTask = async () => {
    ...
    try {
      ...
      const nextTask = (Array.isArray(tasks) ? tasks[0] : null) as
        | SmsTask
        | undefined;
      ...
      if (lastTaskId.current !== nextTask.id) {
        ...
        addLog(
          `Queued task found for ${nextTask.recipient}. Opening SMS composer.`,
          "info",
        );
  ...
  const openSmsComposer = async (task: SmsTask) => {
    ...
    try {
      ...
      addLog(
        `Task ${task.id.slice(0, 8)} handed to Android SMS app. Tap Send to finish.`,
        "success",
      );
      ...
      setStatus("ERROR");
      addLog(
        e.message || "Failed to update task before opening SMS composer.",
        "error",
      );
    }
  };
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-blue-500/20 flex flex-col">
      <header className="sticky top-0 z-20 bg-white/90 backdrop-blur-md border-b border-slate-200 p-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div
            className={`w-3 h-3 rounded-full ${isActive ? "bg-emerald-500 animate-pulse" : "bg-slate-300"}`}
          />
          <div>
            <h1 className="text-sm font-bold tracking-tight text-slate-950 uppercase">
              Mobile Relay Node
            </h1>
      ...
      <main className="flex-1 p-4 space-y-5 max-w-md mx-auto w-full">
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="space-y-1">
              ...
              <p className="text-sm text-blue-50 leading-relaxed">
                This page checks for queued SMS tasks and opens Android's SMS
                app. You still need to tap Send in the composer.
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="bg-white p-4 rounded-lg border border-slate-200 flex flex-col items-center gap-1">
            ...
            <span className="text-xs font-bold text-slate-900">
              {pendingTask ? pendingTask.recipient : "None"}
            </span>
          </div>
        </div>
        ...
        {pendingTask && (
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              <Button
                className="w-full"
                onClick={() => openSmsComposer(pendingTask)}
        ...
        <div className="bg-white p-5 rounded-lg border border-slate-200 space-y-3">
          ...
          <ul className="text-[11px] text-slate-600 space-y-3">
            ...
            <li className="flex gap-2">
              ...
              />
              <span>
                When a task arrives, Android opens your SMS app with the
                recipient and message filled in.
              </span>
            </li>
            <li className="flex gap-2">
              <AlertCircle
              ...
              />
              <span>
                Android requires you to confirm the final send. Fully silent SMS
                sending needs the native mobile app.
              </span>
            </li>
          </ul>
        </div>
